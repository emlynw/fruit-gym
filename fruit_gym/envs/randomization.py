...
def lighting_noise(env: Any) -> None:
    ...
    light_pos_noise = env.np_random.uniform(low=pos_low, high=pos_high, size=3)
    ...
    if random.random() < 0.5:
        ...
        light_diffuse_noise = env.np_random.uniform(low=diffuse_low, high=diffuse_high, size=3)
        light_ambient_noise = env.np_random.uniform(low=ambient_low, high=ambient_high, size=3)
    else:
        light_diffuse_noise = env.np_random.uniform(low=[0.1, 0.1, 0.1], high=[1.0, 1.0, 1.0], size=3)
        light_ambient_noise = env.np_random.uniform(low=[0.0, 0.0, 0.0], high=[0.5, 0.5, 0.5], size=3)

    env.model.light_diffuse[0] = light_diffuse_noise
    env.model.light_ambient[0] = light_ambient_noise

    light_specular_noise = env.np_random.uniform(
    ...
    if random.random() < 0.5:
        light_factor = -1.0
        head_diffuse = env.np_random.uniform(low=0.0, high=0.1, size=3)
        head_ambient = env.np_random.uniform(low=0.0, high=0.1, size=3)
        head_specular = env.np_random.uniform(low=0.0, high=0.1, size=3)
    else:
        light_factor = 1.0
        head_diffuse = env.np_random.uniform(low=0.0, high=0.1, size=3)
        head_ambient = env.np_random.uniform(low=0.0, high=0.1, size=3)
...
def initial_state_noise(env: Any) -> None:
    ...
    ee_noise = env.np_random.uniform(low=ee_noise_low, high=ee_noise_high, size=3)
...
def camera_noise(env: Any) -> None:
    ...
    for cam_name in env.cameras:
        ...
        cam_pos_noise = env.np_random.uniform(low=pos_noise_low, high=pos_noise_high, size=3)
        env.model.body_pos[env.model.body(cam_name).id] = getattr(env, f"{cam_name}_pos") + cam_pos_noise

        cam_quat_noise = env.np_random.uniform(low=quat_noise_range[0], high=quat_noise_range[1], size=4)
...
def floor_noise(env: Any) -> None:
    ...
    """
    floor_tex_id = env.np_random.choice(env.floor_tex_ids)
...
def skybox_noise(env: Any) -> None:
    ...
    """
    skybox_tex_id = env.np_random.choice(env.skybox_tex_ids)
